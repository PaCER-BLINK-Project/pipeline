...
// SORTING - TODO : use sort() from standard library :
#define REPLACE_ELEMS( tab, pos1, pos2 ) { tmp=tab[pos1]; tab[pos1]=tab[pos2]; tab[pos2]=tmp; } 
static void my_sort_float_local( double* ftab, long cnt )
{
        ...
        if(cnt){
                ...
                my_sort_float_local( ftab, end );
                my_sort_float_local( &(ftab[beg]), cnt-beg );
        }
...
{
   // just clean up file 
   FILE* out_f = fopen("total_power.txt","w");
   fclose(out_f);
}
...
void TotalPower::calc( DynamicSpectrum& dynaspec, bool do_dump, bool use_rms, int offset, int ntimes, long int start_time )
{
   ...
   if( do_dump ){
      char szTotalPowerFile[256];
//      sprintf(szTotalPowerFile,"total_power_%05d_%05d.txt",dynaspec.x,dynaspec.y);
      sprintf(szTotalPowerFile,"total_power.txt");
      FILE* out_f = fopen(szTotalPowerFile,"a+");
...
//      printf("DEBUG : saving %d total power values to text file\n",int(size()));
      FILE* out_f = fopen("total_power.txt","a+");
      for(int i=0;i<size();i++){
         TotalPowerRec& rec = (*this)[i];
         fprintf(out_f,"%ld %.4f %.8f %.8f %.8f\n",rec.time_index,rec.total_power,GetMedianOfMedians(),GetMedianOfRMSIQRs(),m_Median);
      }
