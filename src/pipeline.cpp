...
void blink::Pipeline::run(const std::vector<std::shared_ptr<Voltages>>& inputs){
   if(window_offset + batch_size > table_size){
      ...
      get_elements(dm_starttime.data(), imageSize, dm_list.size(), 0, window_start_idx, buffer_size, table_size, dedisp_norm_factor, 59, 629);
      // step 3, clear and rotate the buffer
      clear_buffer(dm_starttime.data(), imageSize, dm_list.size(), window_start_idx, table_size, buffer_size);
