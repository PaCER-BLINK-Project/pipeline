...
echo "------------------------------------------"
echo "./blink_pipeline -c 4 -C 0 -t 1.00s -o allcorr/ -n 2048 -f 169.60 -F 30 -M 20200619163000.metafits -U 1592584240 -w N -v 100 -r -L -G -s calsolutions_chan0_xx.txt -r 1276619416_1276619418_ch133.dat"
./blink_pipeline -c 4 -C 0 -t 1.00s -o allcorr/ -n 2048 -f 169.60 -F 30 -M 20200619163000.metafits -U 1592584240 -w N -v 100 -r -L -G -s calsolutions_chan0_xx.txt -r 1276619416_1276619418_ch133.dat 
echo "------------------------------------------"

max_diff=0.001 # 0.4/0.3 to compare with cotter files 1276619416_20200619163000_vis_real_channel000_time000000_pol0.fits
echo "calcfits_bg xcorr_re.fits = $BLINK_TEST_DATADIR/mwa/1276619416/results/cablegeocorr_applycal/cal_re.fits -p ${max_diff} -p ${max_diff}"
calcfits_bg xcorr_re.fits = $BLINK_TEST_DATADIR/mwa/1276619416/results/cablegeocorr_applycal/cal_re.fits -p ${max_diff} -p ${max_diff}
# repeat to get exit code :
exit_code=0
equal=`calcfits_bg xcorr_re.fits = $BLINK_TEST_DATADIR/mwa/1276619416/results/cablegeocorr_applycal/cal_re.fits -p ${max_diff} -p ${max_diff} | grep "Images are EQUAL" | wc -l`
if [[ $equal -le 0 ]]; then
   exit_code=1
fi


echo "calcfits_bg xcorr_im.fits = $BLINK_TEST_DATADIR/mwa/1276619416/results/cablegeocorr_applycal/cal_im.fits -p ${max_diff} -p ${max_diff}"
calcfits_bg xcorr_im.fits = $BLINK_TEST_DATADIR/mwa/1276619416/results/cablegeocorr_applycal/cal_im.fits -p ${max_diff} -p ${max_diff}
# repeat to get exit code :
exit_code=0
equal=`calcfits_bg xcorr_im.fits = $BLINK_TEST_DATADIR/mwa/1276619416/results/cablegeocorr_applycal/cal_im.fits -p ${max_diff} -p ${max_diff} | grep "Images are EQUAL" | wc -l`
if [[ $equal -le 0 ]]; then
   exit_code=1
fi
